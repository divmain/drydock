...
module.exports = function () {
  ...
  this.server.route({
    ...
    handler: function (request, reply) {
      ...
      try {
        self.route(payload.name).setOption(payload.selectedOption, payload.handler);
        reply("OK");
      } catch (err) {
        if (!(err instanceof Errors.ApiError)) {
          throw err;
  ...
  this.server.route({
    ...
    handler: function (request, reply) {
      ...
      try {
        self.route(payload.name).setOptions(payload.selectedOptions, payload.handler);
        reply("OK");
      } catch (err) {
        if (!(err instanceof Errors.ApiError)) {
          throw err;
  ...
  this.server.route({
    ...
    handler: function (request, reply) {
      ...
      self.delay = request.payload.delay;
      reply("OK");
    }
  });
