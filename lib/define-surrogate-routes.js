...
module.exports = function () {
  ...
  this.server.route({
    ...
    handler: function (request, reply) {
      var routes = _.chain(this.routes)
        .cloneDeep()
        .map(function (route) {
          var handlers = _.map(route.handlers, function (handler) {
            return _.extend({}, handler, { options: _.keys(handler.options) });
          });
  ...
  this.server.route({
    ...
    handler: function (request, reply) {
      var
        ...
        route = _.find(this.routes, { method: payload.method, path: payload.path });
