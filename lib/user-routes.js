...
function updateCookieState (replyObj, originalCookies, modifiedCookies, settings) {
  var
  ...
  _.each(cookiesDiff.add, function (val, key) {
    ...
    replyObj.state(key, modifiedCookies[key], settings);
  });
  _.each(cookiesDiff.del, function (val, key) {
    replyObj.state(key, "", settings);
  });
}
          ...
          .then(function (responseOptions) {
            if (!_.isNull(self.delay)) {
              setTimeout(function () {
                ...
                updateCookieState(r, request.state, handlerContext.cookies, {
                  encoding: self.cookieEncoding,
                  domain: handlerContext.cookieDomain
                });

                _.each(handlerContext.headers, function (val, key) {
                  r.header(key, val);
                });
              }, self.delay);
            }
