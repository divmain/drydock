...
module.exports = {
  ...
  validateSetHandler: function (payload, route) {
    ...
    handler = route.handlers[payload.selectedHandler];

    schema = Joi.object().keys({
      method: Joi.string().allow("*", "GET", "POST", "PUT", "DELETE", "PATCH").required(),
      path: Joi.string().regex(/^[A-Za-z\-\/\_\{\}]+$/).required(),
      selectedHandler: Joi.string().valid(_.keys(route.handlers)).required(),
      selectedOption: Joi.string().valid(_.keys(handler && handler.options)),
      selectedOptions: Joi.array().valid(_.keys(handler && handler.options))
    }).without("selectedOption", "selectedOptions");
  ...
  validateSetOption: function (payload, route) {
    ...
    schema = Joi.object().keys({
      ...
      selectedOption: Joi.string().valid(_.keys(handler && handler.options)).required()
    });
